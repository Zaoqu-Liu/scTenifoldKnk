...
#' networks <- makeNetworksFast(X = X, nNet = 10, nCells = 500, nComp = 3, 
...
makeNetworksFast <- function(X, 
  ...
  if (verbose) {
    ...
    cat(sprintf("Each network: %d genes, %d cells (subsampled), %d components\n", 
                nGenes, nCells, nComp))
  }
  ...
  buildOneNetwork <- function(net_idx) {
    ...
    if (nrow(X_sub) < nComp + 1) {
      warning(sprintf("Network %d has too few genes (%d) for nComp=%d, skipping", 
  ...
  if (nCores > 1 && nNet > 1) {
    ...
    parallel::clusterExport(cl, 
                           varlist = c("X_matrix", "geneList", "nGenes", "nComp", 
