test_that("makeNetworksFast produces correct number of networks", {
  ...
  networks <- makeNetworksFast(X, nNet = 5, nCells = 100, nComp = 3,
...
test_that("makeNetworksFast parallel vs sequential gives same results", {
  ...
  # Sequential
  set.seed(456)
  nets_seq <- makeNetworksFast(X, nNet = 3, nCells = 100, nComp = 3,
                               nCores = 1, verbose = FALSE, seed = 456)
  
  # Parallel (if cores available)
  if(parallel::detectCores() > 1){
    set.seed(456)
    nets_par <- makeNetworksFast(X, nNet = 3, nCells = 100, nComp = 3,
